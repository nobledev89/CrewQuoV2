...
- [x] **Money boundary.** Built 2026-08-19 against `docs/operating-model/money-boundary.md`, the packet rates and invoices never had until the boundary reopened both: migration `0013_money_boundary.sql` adds `projects.reporting_currency` (snapshotted from the owner company at creation, never read live, so changing a company currency cannot restate a closed project) and the human-entered `fx_rates` ledger — required source, no update path in the API or the database, delete refused once a frozen snapshot cites the row. `packages/shared/src/money.ts` converts in exact `bigint` arithmetic with a single rounding (§41.9) and picks the latest rate on or before the money's date, never a future one and never a reversed one. The PAY snapshot freezes its currency and its rate at submit alongside the amount; project summaries report in the project's unit and withhold what no rate covers, naming the pair, earliest date and count in `conversionGaps` and withholding margin with it. Unlike PAY currencies are now allowed where a rate exists, refused with the missing row named where one does not. A project invoice is denominated in its project's reporting currency and **refuses** an unlike BILL card rather than converting it, because an agreed charge-out rate is what the client owes. Tax was a definition and is delivered as one: the packet enumerates tax identity, place-of-supply addresses, per-line tax with a stated rounding rule, credit notes and payment allocation, no tax schema ships, and `TAX_IS_MANUAL_NOTICE` keeps the product from calling the document a tax invoice.
- [~] **Durable delivery foundation:** migration `0012_durable_delivery.sql`, transactional outbox, signature-verified/deduplicated webhook inbox persistence, SKIP LOCKED leases, bounded worker runner, exponential retry, dead letters and reason-required audited replay are built. Company creation commits its first domain event atomically and the audit purge now runs only through an external scheduler. Remaining adoption: connect the notification handlers and retrofit shared idempotency/outbox emission across the older create/submit/approve mutations. The merchant-of-record handler is no longer a near-term consumer — that integration is deferred to the end of the phase.
