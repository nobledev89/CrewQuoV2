...
- ~~**Exact seed pricing per currency**~~ — **answered 2026-08-20: USD only.** CrewQuo sells in one currency and localises none, so `plan_prices` holds one USD row per plan per interval and the amounts are data entered through the super-admin price editor rather than a decision that blocks it. Consistent with holding no exchange rate anywhere in the product (2026-08-19): a non-USD price could never have been derived, only set and maintained by hand. Note that a *company's* currency label stays user-changeable and independent of what it is charged; there is nothing to convert between them and nothing should be built that tries.
- ~~**Which merchant of record**~~ — **answered 2026-08-20: Paddle.** The acceptance bar is unchanged and still gates the integration: seller KYC and payout setup (Philippine bank payout is the first thing to check), and a test membership proving purchase, renewal, failed payment, cancellation, refund and replay-safe webhook/API reconciliation. Paddle is a merchant of record rather than a gateway, which is the reason to pick one — it owns VAT/sales-tax determination and remittance, so §30's tax gate stays *defined, deliberately not built*, and CrewQuo neither computes nor stores tax. Webhooks land in the existing `webhook_inbox` signature-verified and deduplicated, with the entitlement snapshot committed in the same transaction as the subscription state change (§36, decision #25).
- ~~**Company data export**~~ — **answered 2026-08-20: free and ungated**, for every plan including free `crew`, so there is no entitlement key and no plan check. Format is a machine-readable bundle (JSON plus CSV per table), not PDF.
