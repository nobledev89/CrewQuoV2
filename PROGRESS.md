...
Last updated: 2026-08-18 · Current phase: **Phase 6 — commercial readiness** (invoices, commercial agreements, three-view workspace, Platform admin, guarded company creation and the durable-delivery substrate complete; older-domain adoption, money boundary, billing, notifications and launch readiness remain) · Phases 0–5 built and verified. **Phase 5's coherence judgement was answered on 2026-08-18** by a measured audit of all 31 screens, and the information-architecture and density work it produced shipped as Phase 5.5

> **Re-run the end-to-end verification any time:** `pnpm --filter @crewquo/api verify:e2e` (needs the DB up, migrations + seed applied, and the API running). **418 checks** covering currency, label rules, the Phase 3/4 core-loop numbers, exports, malformed identifiers, migration backfills, portal isolation, metering, platform administration, members, profile, invoices, commercial agreements, guarded company creation and the durable-delivery substrate (the acceptance scripts in the operating-model packets). The script itself is now included in API TypeScript checking. Checked in at [apps/api/scripts/verify-e2e.ts](apps/api/scripts/verify-e2e.ts).
>
...
- [~] **Durable delivery foundation:** `0012_durable_delivery.sql` plus transactional outbox, verified/deduplicated webhook inbox persistence, SKIP LOCKED leases, bounded worker runner, exponential retry, dead letters, Platform Operations health and reason-required audited replay are built. Company creation emits atomically; audit purge is external-scheduler-only. Remaining: connect real consumers and retrofit shared idempotency/outbox emission across older create/submit/approve paths.
