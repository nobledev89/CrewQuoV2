...
Last updated: 2026-08-18 · Current phase: **Phase 6 — commercial readiness** (invoices, commercial agreements, the three-view workspace, the CrewQuo Platform admin workspace and the company ownership/creation safeguard complete; money boundary, billing, durable delivery, notifications and launch readiness remain) · Phases 0–5 built and verified. **Phase 5's coherence judgement was answered on 2026-08-18** by a measured audit of all 31 screens, and the information-architecture and density work it produced shipped as Phase 5.5

> **Re-run the end-to-end verification any time:** `pnpm --filter @crewquo/api verify:e2e` (needs the DB up, migrations + seed applied, and the API running). **410 checks** covering currency, label rules, the Phase 3/4 core-loop numbers, the export engine, malformed identifiers, all three migration backfills, the portal, the placeholder/meter rules, the super-admin companies console, member management, the profile endpoint, invoices, the whole commercial-agreement domain and the whole company-creation safeguard (the acceptance scripts in §12 of both operating-model packets). Previous phases' scripts were ad-hoc and lost; this one is checked in at [apps/api/scripts/verify-e2e.ts](apps/api/scripts/verify-e2e.ts).
>
...
- [x] **Company ownership/creation safeguard (§3.1.1)** (2026-08-18): the identity domain reopened, so it got the §19.5 packet it never had — [company-creation.md](docs/operating-model/company-creation.md). Migration `0011_company_creation_safeguard.sql` adds legal identity (`country`, `registration_id` + a generated normalised column) to `companies`, the permanently ledgered `company_creation_allowances`, the six-state `company_creation_requests` machine, and the identity-keyed `trial_grants`. `POST /v1/company-creation-requests` files the advanced flow, `/v1/admin/company-creation-requests` decides it, and `POST /v1/me/companies` serves both authorities — spending whichever applies inside the transaction that creates the company. Web: the profile section is now three states chosen by the server, the Platform console gained a review queue, and platform settings gained the two policy flags.
  ...
  **The rules worth restating, because they are decisions rather than implementation:**
  ...
  - [x] **Verified:** 266 unit tests green (**48 new** — the state machine, the creation gate, identifier normalisation, the duplicate classifier, request routing and trial eligibility), all 5 packages type-check, production build clean, **410 live-Postgres checks** green (**92 new**, implementing the packet's §12 acceptance script) and **26 browser tests** green (**4 new**, walking the included company, a duplicate routed to recovery, a request approved in the Platform console and consumed, and a refused second trial).

> **Creating a company and immediately subscribing it served free-plan entitlements for a minute — caught by the e2e on 2026-08-18.** The first version wrote a company-scoped `company.created` audit row, and `recordAudit` resolves the company's entitlements to read its `audit_retention_days`. At the instant of creation a company has no subscription, so it resolves to the free plan — whose retention is `0`, meaning the row could **never** be written whatever the company later paid for. The only real effect was the 60-second memoisation left behind, which made a freshly provisioned paid company read as `crew` until the TTL lapsed. The row is gone and `company.created` is not in `AUDIT_ACTIONS`; creation is recorded in `platform_audit_logs`, which has no retention rule and no cache. Worth remembering generally: **calling `recordAudit` inside the transaction that creates a company poisons that company's entitlement cache.**
