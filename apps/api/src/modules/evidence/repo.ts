...
const FIELDS = `
  ...
  e.asset_id, e.asset_movement_id,
...
export async function listEvidence(
  ...
  if (filter.assetMovementId) {
    params.push(filter.assetMovementId);
    where += ` and e.asset_movement_id = $${params.length}`;
  }
    ...
    `with inserted as (
       insert into project_evidence
         (project_id, company_id, file_id, category, caption, notes, evidence_date,
          captured_at, location_id, diary_entry_id, asset_id, asset_movement_id,
          sort_order, uploaded_by_user_id, batch_client_id)
       values ($1,$2,$3,$4,$5,$6,$7::date,$8::timestamptz,$9,$10,$11,$12,$13,$14,$15)
...
const PATCH_COLUMNS: Record<keyof EvidencePatch, { column: string; cast: string }> = {
  ...
  assetId: { column: 'asset_id', cast: '::uuid' },
  assetMovementId: { column: 'asset_movement_id', cast: '::uuid' },
